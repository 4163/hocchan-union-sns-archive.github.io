...
cwd = os.getcwd()
excel_file = os.path.join(cwd, 'miss_monochro_combined.xlsx')  # Adjusted the filename to your context
...
def format_text(text):
    ...
    wrapped_text = re.sub(
        r'(https://\S+)',
        r'<span class="pink-link"><a href="twitter-miss_monochro.html">\1</a></span>',
        text
    )
    ...
    wrapped_text = re.sub(
        ...
        r'\1<span class="pink-link"><a href="twitter-miss_monochro.html">\2</a></span>',
        wrapped_text
    )

    # Handle mentions with an exception for @miss_monochro
    wrapped_text = re.sub(
        r'(^|\s)(@[a-zA-Z0-9_]+)',  
        lambda match: match.group(0) if match.group(2) == '@miss_monochro' else f'{match.group(1)}<span class="pink-link"><a href="twitter-miss_monochro.html">{match.group(2)}</a></span>',
        wrapped_text
    )
...
for index, row in df.iterrows():
    ...
    if row['Post Type'] == 'image':
        ...
        image_tags = ''.join([f'<img src="Media/@miss_monochro/{date_filename}{f"_{i+1}" if i > 0 else ""}.jpg" />' for i in range(len(images))])
    ...
    tweet_contents = f'''
            <span class="tweet-contents">
                <span style="margin-bottom: 2px;"><a href="twitter-miss_monochro.html"><strong>ミス・モノクローム</strong></a></span>&nbsp;
                <span style="color: #536471; margin-bottom: 2px;">@miss_monochro&nbsp;</span><span style="color: #536471; margin-bottom: 2px; margin-top: 5px;" class="date-margin">
    ...
    <div class="post-contents tweet-{tweet_count:04d}">
        ...
        <a href="twitter-miss_monochro.html">
            <div class="post-profile">
                <img src="Media/@miss_monochro/XHMFikXS_400x400.jpeg" />
            </div>
        </a>
