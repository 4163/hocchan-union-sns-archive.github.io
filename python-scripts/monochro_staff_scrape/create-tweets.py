...
cwd = os.getcwd()
excel_file = os.path.join(cwd, 'monochro_staff_combined.xlsx')  # Adjusted the filename to your context
...
def format_text(text):
    ...
    wrapped_text = re.sub(
        r'(https://\S+)',
        r'<span class="pink-link"><a href="twitter-monochro_staff.html">\1</a></span>',
        text
    )
    ...
    wrapped_text = re.sub(
        ...
        r'\1<span class="pink-link"><a href="twitter-monochro_staff.html">\2</a></span>',
        wrapped_text
    )

    # Handle mentions with an exception for @monochro_staff
    wrapped_text = re.sub(
        r'(^|\s)(@[a-zA-Z0-9_]+)',  
        lambda match: match.group(0) if match.group(2) == '@monochro_staff' else f'{match.group(1)}<span class="pink-link"><a href="twitter-monochro_staff.html">{match.group(2)}</a></span>',
        wrapped_text
    )
...
for index, row in df.iterrows():
    ...
    if row['Post Type'] == 'image':
        ...
        image_tags = ''.join([f'<img src="Media/@monochro_staff/{date_filename}{f"_{i+1}" if i > 0 else ""}.jpg" />' for i in range(len(images))])
